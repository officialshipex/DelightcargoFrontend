...
const ActionRequired = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${REACT_APP_BACKEND_URL}/order/ndr`, {
        params,
            ...
            {mobileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-sm z-[60] text-[10px] font-[600] overflow-hidden animate-popup-in">
                <ul className="py-1">
                  ...
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer" onClick={() => { setIsBulkNdrModalOpen(true); setMobileDropdownOpen(false); }}>Bulk NDR Action</li>
                </ul>
              </div>
            )}
          </div>
