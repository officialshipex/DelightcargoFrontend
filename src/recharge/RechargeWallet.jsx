...
const RechargeWallet = () => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-[#0192ED] p-6 text-center relative">
        <button
          onClick={() => navigate(-1)}
          className="absolute left-6 top-6 text-white hover:text-blue-100 transition-colors"
        >
          <FiArrowLeft size={18} />
        </button>
        ...
        <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 w-[90%] bg-white rounded-xl shadow-lg p-4 flex justify-between items-center z-10">
          ...
          <div className="bg-blue-50 p-3 rounded-full">
            <FiCreditCard className="text-[#0192ED] sm:text-[14px] text-[12px]" />
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                type="number"
                className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg sm:text-[14px] text-[12px] font-bold text-gray-700 focus:outline-none focus:border-[#0192ED] focus:ring-1 focus:ring-blue-50 transition-all placeholder-gray-300"
                min={1000}
                value={amount}
            ...
            {[1000, 2500, 5000, 10000].map((val) => (
              <button
                ...
                className={`py-2 px-1 rounded-lg sm:text-[12px] text-[10px] font-semibold transition-all duration-200 border ${Number(amount) === val
                  ? "bg-[#0192ED] text-white border-[#0192ED] shadow-md transform scale-105"
                  : "bg-white text-gray-600 border-gray-200 hover:border-[#0192ED] hover:bg-blue-50"
                  }`}
              >
                ₹ {val}
              </button>
          ...
          <div className="pt-4 space-y-2">
            <button
              ...
              className={`w-full py-2 rounded-lg font-bold sm:text-[12px] text-[10px] text-white shadow-sm transition-all duration-300 transform active:scale-95 ${Number(amount) < 1000
                ...
                : "bg-[#0192ED] hover:bg-blue-600 hover:shadow-xl shadow-blue-200"
                }`}
            >
