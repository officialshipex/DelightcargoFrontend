...
const NewOrder = ({ userId: initialUserId }) => {
  ...
  const [bulkUserId, setBulkUserId] = useState(null);
  ...
  const handleSavePackageDetails = async (details) => {
    SavePackageDetails({ details, selectedOrders, setRefresh, refresh });
  ...
  const handleBulkShip = async () => {
    ...
    try {
      const checkResponse = await axios.get(
        `${REACT_APP_BACKEND_URL}/order/checkBulkUser`,
        {
      ...
      if (!checkResponse.data.success) {
        Notification(checkResponse.data.message || "Selected orders belong to multiple users.", "error");
        return;
      }

      setBulkUserId(checkResponse.data.userId);

      const pickupCheckResponse = await axios.get(
        `${REACT_APP_BACKEND_URL}/order/checkBulkPickup`,
        {
          params: { orderIds: selectedOrders },
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      const { showPopup, orders: pickupOrders } = pickupCheckResponse.data;
      ...
      if (!showPopup) {
        ...
        const { success, message, successCount, failureCount } = shipResponse.data;

        if (success) {
          Notification(
            message || `${successCount} orders shipped successfully, ${failureCount} failed.`,
            "success"
          );
        } else {
      ...
      setSelectedData(pickupOrders);

    } catch (error) {
      console.error("Bulk Ship Error:", error);
      Notification(
        error.response?.data?.message ||
        "Something went wrong while processing bulk shipment.",
        "error"
      );
    }
  };

  const handleBulkUpdatePickup = () => {
    setShowBulkShipModal(true);
  ...
  const handleCancel = (orderId) => {
    cancelOrder({ orderId, refresh, setRefresh });
  };
  ...
  return (
    <div className="w-full">
              ...
              <div className="absolute right-0 mt-1 w-56 text-[10px] bg-white border border-gray-200 shadow-sm z-[60] font-[600] overflow-hidden animate-popup-in">
                <ul className="py-1">
                  ...
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer flex items-center gap-2"
                    ...
                    <Package className="w-3 h-3" /> Update Package Details
                  </li>
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer flex items-center gap-2"
                    onClick={() => { handleBulkShip(); setDesktopDropdownOpen(false); }}>
                    <Truck className="w-3 h-3" /> Bulk Ship
                  </li>
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer flex items-center gap-2"
                    onClick={() => { handleBulkUpdatePickup(); setDesktopDropdownOpen(false); }}>
                    <MapPin className="w-3 h-3" /> Update Pickup Address
                  </li>
                  <li className="px-3 py-2 text-red-600 hover:bg-red-50 cursor-pointer flex items-center gap-2 font-[700]"
                    onClick={() => { handleBulkDelete(); setDesktopDropdownOpen(false); }}>
      ...
      <div ref={tableRef} className="hidden md:block">
        <div style={{ height: tableHeight }} className="overflow-auto relative bg-white shadow-sm">
          <OrdersTable
            orders={orders}
            loading={loading}
            ...
            {mobileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-sm z-[60] text-[10px] font-[600] overflow-hidden animate-popup-in">
                <ul className="py-1">
                  ...
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer" onClick={() => { setShowPackageModal(true); setMobileDropdownOpen(false); }}>Update Package Details</li>
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer" onClick={() => { handleBulkShip(); setMobileDropdownOpen(false); }}>Bulk Ship</li>
                  <li className="px-3 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer" onClick={() => { handleBulkUpdatePickup(); setMobileDropdownOpen(false); }}>Update Pickup Address</li>
                  <li className="px-3 py-2 text-red-600 hover:bg-red-50 cursor-pointer" onClick={() => { handleBulkDelete(); setMobileDropdownOpen(false); }}>Bulk Delete</li>
                </ul>
              </div>
            )}
          </div>
      ...
      {showPackageModal && (
        <UpdatePackageDetails
          ...
          onUpdate={handleSavePackageDetails}
        />
      )}
