...
export default function CreateNewCourier({ isSidebarAdmin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData._id) {
        await axios.put(`${REACT_APP_BACKEND_URL}/b2b/courierServices/updateCourier/${formData._id}`, formData);
      } else {
        await axios.post(`${REACT_APP_BACKEND_URL}/b2b/courierServices/createCourier`, formData);
      }
      ...
      Notification(error?.response?.data?.error || "Error saving courier", "error");
      ...
      <div className="max-w-full sm:px-2 mx-auto">
        <div className="bg-white px-3 py-2 rounded-lg shadow-sm border border-gray-100 mb-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-2">
              {/* Provider */}
              <CustomDropdown
                label="Provider"
                name="provider"
              ...
              {/* Weight */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] sm:text-[12px] font-[600] text-gray-700">Weight</label>
                <input
                  type="text"
                  name="weight"
                  placeholder="Weight"
