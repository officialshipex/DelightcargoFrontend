...
const CourierComparisonSwapped = ({ selectedUserId, refresh, selectedDateRange }) => {
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col text-gray-500 items-center justify-center py-24 px-4 text-center">
      ...
      <button
        onClick={handleShip}
        className="mt-6 px-3 py-2 bg-[#0192ED] text-white text-[12px] font-[600] rounded-lg hover:bg-blue-500 transition"
      >
        Ship Now
      </button>
    </div>
    ...
    <div className="w-full overflow-x-hidden box-border">
      ...
      <div ref={tableRef} style={{ height: tableHeight }} className="hidden md:block overflow-auto relative">
        <table className="min-w-full text-[12px]">
          <thead className="sticky top-0 z-10 bg-[#0192ED]">
      ...
      <div ref={mobileRef} style={{ height: tableHeight }} className="md:hidden w-full max-w-screen-sm mx-auto space-y-2 overflow-y-auto">
        {loading ? (
          ...
          couriers.map((courier, idx) => (
            ...
            >
              {/* Header: Logo + Name */}
              <div className="flex items-center bg-blue-100 rounded-lg p-2 gap-2 mb-2">
                <img
                  src={getCarrierLogo(courier.courier) || "https://via.placeholder.com/40"}
                  alt={courier.courier}
              ...
              <div className="grid grid-cols-2 gap-2 text-[12px] mb-2">
                {[
                  ...
                  { label: "Delivered", value: courier.delivered, color: "text-[#0192ED]" },
                  ...
                  { label: "NDR Delivered", value: courier.ndrDelivered, color: "text-[#0192ED]" },
                  { label: "Lost/Damaged", value: courier["Lost/Damaged"], color: "text-gray-700" },
                ].map((item, idx) => (
                  <div key={idx} className="bg-gray-50 border border-[#0192ED] rounded-lg px-2 py-1">
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-3">
                  {["Zone A", "Zone B", "Zone C", "Zone D", "Zone E"].map((zone) => (
                    <div
                      key={zone}
                      className="flex flex-col border border-[#0192ED] items-center justify-center px-2 py-1 rounded-lg bg-[#F5F7FA] text-center text-[10px] font-[500] text-gray-700 shadow-sm"
                    >
                      <div className="text-gray-500 text-[10px]">{zone}</div>
                      <div className="text-[10px] font-[600] text-[#0192ED]">
