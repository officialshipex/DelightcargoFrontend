...
const Dashboard = ({ selectedUserId, selectedDateRange }) => {
    ...
    const Card = ({ title, value, subtitle, bgColor = "bg-white" }) => (
        <div className={`flex items-center gap-2 p-4 rounded-lg shadow-sm border hover:border-[#0192ED] transition-all duration-700 ${bgColor} w-full`}>
            <div className="bg-[#0192ED] p-2 rounded-full">
    ...
    const StatBox = ({ label, value, icon: Icon, onClick }) => (
        <div className="flex items-center border border-gray-200 cursor-pointer bg-white rounded-lg p-3 shadow-sm hover:border-[#0192ED] transition-all duration-700 w-full" onClick={onClick}>
            {Icon && (
                <div className="bg-[#0192ED] p-2 rounded-full mr-3">
