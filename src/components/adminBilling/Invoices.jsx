...
const Invoices = () => {
  ...
  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      case "paid":
        return "bg-blue-100 text-[#0192ED] border-[#0192ED]/20";
      case "pending":
  ...
  const renderUserDetails = (inv) => {
    ...
    return (
      <div className="flex flex-col">
        <span className="text-[#0192ED] font-medium leading-tight">{user.userId || ""}</span>
      ...
      <div className="hidden md:flex gap-2 relative sm:items-center">
        ...
        <div className="relative flex-1 max-w-[240px]">
          ...
          <input
            ...
            className="w-full h-9 pl-8 pr-3 font-[600] text-[12px] border border-gray-300 rounded-lg focus:outline-none focus:border-[#0192ED] bg-white text-gray-700 placeholder-gray-400 transition-all shadow-sm"
          />
        </div>
        ...
        <div className="flex items-center gap-2 ml-auto" ref={actionRef}>
          ...
          <div className="relative">
            <button
              ...
              className={`h-9 px-3 rounded-lg text-[12px] font-[600] flex items-center gap-1 border transition-all ${selectedInvoices.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
              Actions
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => { setInvoiceNumber(e.target.value); setPage(1); }}
              placeholder="Search Invoice No."
              className="w-full h-[32px] pl-7 pr-2 text-[10px] font-[600] border border-gray-300 rounded-lg focus:outline-none focus:border-[#0192ED] bg-white text-gray-700 placeholder-gray-400 transition-all shadow-sm"
            />
          </div>
          <button
            onClick={() => setIsFilterPanelOpen(true)}
          ...
          >
            <Filter className="w-3.5 h-3.5 text-[#0192ED]" />
            More Filters
          </button>
        </div>
        ...
        <div className="h-[calc(100vh-235px)] overflow-y-auto bg-white overflow-hidden">
          <table className="w-full text-left border-collapse text-[12px] relative">
            <thead className="sticky top-0 z-40 bg-[#0192ED] text-white font-[600]">
              <tr>
                <th className="py-2 px-3 w-10 shadow-[0_1px_0_0_#0192ED]">
                ...
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">User Details</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Invoice No</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Shipments</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Amount Details</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Created On</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Invoice Period</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Status</th>
                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Action</th>
              </tr>
            </thead>
                  ...
                  <tr key={inv._id} className="hover:bg-gray-50 text-[12px] transition-colors border-b">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1 group">
                        <span className="font-medium text-[#0192ED]">{inv.invoiceNumber}</span>
                      ...
                      <div className="flex items-center gap-2">
                        {inv.invoiceUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center justify-center bg-[#0192ED] text-white w-8 h-8 rounded-lg hover:shadow-md hover:bg-opacity-90 transition-all shadow-sm"
        ...
        <div className="p-2 justify-between bg-white rounded-lg flex gap-2 items-center border border-gray-100 mb-2 shadow-sm">
          ...
          <div className="relative" ref={actionRef}>
            <button
              disabled={selectedInvoices.length === 0}
              onClick={() => setActionOpen(!actionOpen)}
              className={`h-[30px] px-3 rounded-lg flex items-center justify-center border transition-all ${selectedInvoices.length > 0 ? "border-[#0192ED] text-[#0192ED] bg-white shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
                ...
                <div key={inv._id} className="bg-white rounded-lg shadow-sm border border-gray-200 px-3 py-2 animate-popup-in">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1">
                          <span className="font-medium text-[#0192ED] text-[10px]">{inv.invoiceNumber}</span>
                          <button onClick={() => handleCopy(inv.invoiceNumber, inv._id + '_inv_mob')}>
                            {copiedId === inv._id + '_inv_mob' ? <FiCheck className="w-2.5 h-2.5 text-blue-500" /> : <FiCopy className="w-3 h-3 text-gray-400" />}
                          </button>
                        </div>
                    ...
                    <div className="text-right">
                      <div className="font-bold text-[#0192ED] text-[11px]">₹{inv.amount}</div>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-50">
                    <div className="flex items-center gap-2 w-full">
                      <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center font-bold text-[#0192ED] text-[12px] shrink-0 border border-gray-300">
                      ...
                      <div className="min-w-0 leading-tight flex justify-between items-center w-full">
                        ...
                        <p className="text-[#0192ED] font-medium text-[10px] truncate">{user.userId}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 ml-2">
                      {inv.invoiceUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="p-1.5 bg-[#0192ED] text-white rounded-lg flex items-center justify-center shadow active:scale-95 transition-transform"
                          title="Download Invoice PDF"
                        >
                          <HiOutlineDownload className="w-4 h-4" />
                        </a>
