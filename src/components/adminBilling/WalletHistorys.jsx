                ...
                <div className="flex items-center gap-2 ml-auto" ref={actionRef}>
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`h-9 px-3 rounded-lg text-[12px] font-[600] flex items-center gap-1 border transition-all ${selectedTransactions.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                                }`}
                        >
                            Actions
                        ...
                        {actionOpen && (
                            <div className="absolute right-0 mt-1 bg-white border-2 border-gray-100 rounded-lg shadow-xl w-44 text-[12px] z-[100] animate-popup-in overflow-hidden">
                                <div
                                    className="px-4 py-2 hover:bg-blue-50 cursor-pointer font-[600] text-gray-600 border-b border-gray-50"
                    ...
                    <table className="w-full text-[12px] text-left border-collapse relative">
                        <thead className="sticky top-0 z-40 bg-[#0192ED] text-white font-[600]">
                            <tr>
                                ...
                                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Date</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Transaction ID</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Amount</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Status</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_#0192ED]">Description</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={row._id} className="hover:bg-gray-50 text-[12px] transition-colors border-b border-gray-200">
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex flex-col text-gray-700">
                                                <span className="text-[#0192ED] font-medium leading-tight">{row.user.userId}</span>
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex items-center gap-1 group">
                                                <span className="text-[#0192ED] font-medium">{row.transactionId}</span>
                                        ...
                                        <td className="px-3 py-2">
                                            <span className={`px-2 py-0.5 rounded text-[10px] ${row.status === "success" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>
                                                {row.status}
                                            </span>
                                        </td>
                                        <td className="px-3 py-2 text-gray-700">
                                                    ...
                                                    <div className="flex items-center gap-2 group">
                                                        ...
                                                        {row.paymentId && (
                                                            <button onClick={() => handleCopy(row.paymentId, row._id + '_pay')}>
                                                                {copiedId === row._id + '_pay' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-300 transition-opacity opacity-0 group-hover:opacity-100" />}
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
                ...
                <div className="p-2 gap-2 bg-white rounded-lg flex justify-between items-center border border-gray-100 mb-2 shadow-sm">
                    ...
                    <div className="relative" ref={actionRef}>
                        <button
                            disabled={selectedTransactions.length === 0}
                            onClick={() => setActionOpen(!actionOpen)}
                            className={`h-[30px] px-3 rounded-lg flex items-center justify-center border transition-all ${selectedTransactions.length > 0 ? "border-[#0192ED] text-[#0192ED] bg-white shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                                }`}
                        >
                        ...
                        {actionOpen && (
                            <div className="absolute right-0 mt-1 bg-white border-2 border-gray-100 rounded-lg shadow-xl w-40 text-[10px] z-[100] animate-popup-in overflow-hidden">
                                <div
                                    className="px-3 py-2 hover:bg-blue-50 cursor-pointer font-[600] text-gray-600 border-b border-gray-50"
                                    onClick={handleExport}
                                >
                                    Export Excel
                                </div>
                                ...
                                <div className="flex justify-between mb-2 items-start relative">
                                    ...
                                    <div className="pl-6 space-y-1">
                                        <div className="flex items-center gap-1 font-[600] text-gray-700">
                                            <span className="text-[#0192ED] truncate max-w-[120px] font-bold">{row.transactionId}</span>
                                            <button onClick={() => handleCopy(row.transactionId, row._id + '_txn_mobile')}>
                                                {copiedId === row._id + '_txn_mobile' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400" />}
                                            </button>
                                        </div>
                                    ...
                                    <div className="text-right">
                                        <p className="font-bold text-[#0192ED] text-[10px]">₹{Number(row.amount).toFixed(2)}</p>
                                        <span className={`px-2 py-0.5 rounded text-[10px] inline-block ${row.status === "success" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>
                                            {row.status}
                                        </span>
                                    </div>
                                </div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {row.paymentId && (
                                                <button onClick={() => handleCopy(row.paymentId, row._id + '_pay_mobile')}>
                                                    {copiedId === row._id + '_pay_mobile' ? <FiCheck className="w-2.5 h-2.5 text-[#0192ED]" /> : <FiCopy className="w-2.5 h-2.5 text-gray-400" />}
                                                </button>
                                            )}
                                        </div>
                                    </div>
