      ...
      <div className="text-[12px] font-[600]">
        ...
        <div className="hidden md:grid md:grid-cols-4 gap-2 my-2">
          {summaryItems.map((item, idx) => (
            <div
              key={idx}
              className="flex items-center justify-start text-start gap-4 p-2 bg-white rounded-lg border border-[#0192ED] hover:shadow-sm transition-shadow duration-300"
            >
              <div className="bg-[#0192ED] text-white p-2 rounded-full">
                {item.icon}
              </div>
        ...
        <div className="flex items-center gap-2 ml-auto" ref={actionRef}>
          ...
          <div className="relative">
            <button
              ...
              className={`h-9 px-3 rounded-lg text-[12px] font-[600] flex items-center gap-1 border transition-all ${selectedRemittances.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
              Actions
                ...
                transactions.map((row, index) => (
                  <tr key={index} className="border-t border-gray-300 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3 text-gray-700 w-[180px] max-w-[180px]">
                      <p className="text-[#0192ED] font-medium truncate">
                        {row.user?.userId}
                      </p>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-1 group">
                        <span className="text-[#0192ED] font-medium cursor-pointer hover:underline" onClick={() => openRemittanceDetails(row.remittanceId)}>
                          {row.remittanceId}
                        </span>
                        <button onClick={() => handleCopy(row.remittanceId, row._id + '_remId')}>
                          {copiedId === row._id + '_remId' ? <FiCheck className="text-blue-500" /> : <FiCopy className="text-gray-400 opacity-0 group-hover:opacity-100" />}
                        </button>
                      </div>
                    </td>
                    <td className="py-2 px-3 text-[#0192ED] font-medium">
                      <div className="flex items-center gap-1 group">
                        <span>{row.utr || "N/A"}</span>
                        {row.utr && (
                          <button onClick={() => handleCopy(row.utr, row._id + '_utr')}>
                            {copiedId === row._id + '_utr' ? <FiCheck className="text-blue-500" /> : <FiCopy className="text-gray-400 opacity-0 group-hover:opacity-100" />}
                          </button>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3 font-medium text-[#0192ED]">{row.remittanceInitiated?.toFixed(2)}</td>
                    <td className={`py-2 px-3`}>
                      <p className={`rounded px-2 py-0.5 text-[10px] text-center ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "text-red-600 bg-red-100"}`}>{row.status}</p>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="flex items-center justify-between gap-2 bg-white p-2 rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="relative" ref={actionRef}>
            <button
              disabled={selectedRemittances.length === 0}
              onClick={() => setActionOpen(!actionOpen)}
              className={`h-[30px] px-3 rounded-lg flex items-center justify-center border transition-all ${selectedRemittances.length > 0 ? "border-[#0192ED] text-[#0192ED] bg-white shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
              ...
              <div key={row._id} className={`bg-white rounded-lg shadow-sm animate-popup-in border border-gray-200 px-3 py-2 relative ${detailsPopupId === row.remittanceId ? 'z-50' : 'z-10'}`}>
                <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center gap-1 group">
                            <span className="font-[600] text-gray-700 text-[10px] hover:underline cursor-pointer" onClick={() => openRemittanceDetails(row.remittanceId)}>
                              Remmitance Id : <span className="font-bold text-[#0192ED]">{row.remittanceId}</span>
                            </span>
                            <button onClick={() => handleCopy(row.remittanceId, row.remittanceId + '_remId_mob')}>
                              {copiedId === row.remittanceId + '_remId_mob' ? <FiCheck className="w-2.5 h-2.5 text-blue-500" /> : <FiCopy className="w-2.5 h-2.5 text-gray-300" />}
                            </button>
                          </div>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[10px] whitespace-nowrap ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>
                          {row.status}
                        </span>
                      </div>
                  ...
                  <div className="text-right flex flex-col items-end">
                    <div className="flex flex-col items-end">
                      ...
                      <p className="font-bold text-[#0192ED] text-[10px] tracking-tight">₹{row.remittanceInitiated?.toFixed(2)}</p>
                      <p
                        className="text-[9px] text-[#0192ED] font-bold border-b border-dashed border-[#0192ED] cursor-pointer hover:opacity-80"
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px] mb-2">
                  <div>
                    ...
                    <div className="flex items-center gap-1 group">
                      <p className="font-bold text-[#0192ED]">{row.utr || "N/A"}</p>
                      {row.utr && row.utr !== "N/A" && (
                        <button onClick={() => handleCopy(row.utr, row.remittanceId + '_utr_mob')}>
                          {copiedId === row.remittanceId + '_utr_mob' ? <FiCheck className="w-2.5 h-2.5 text-blue-500" /> : <FiCopy className="w-2.5 h-2.5 text-gray-300" />}
                        </button>
                      )}
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="font-bold text-[#0192ED]">{row.remittanceMethod || "N/A"}</p>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="space-y-1.5 text-[10px]">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="text-[#0192ED] font-bold">₹{row.amountCreditedToWallet?.toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between border-t border-gray-50 pt-1">
                            ...
                            <span className="text-[#0192ED] font-bold">₹{row.remittanceInitiated?.toFixed(2)}</span>
                          </div>
                        </div>
                      </motion.div>
                    </>
                ...
                <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
                  <div className="flex w-full items-center gap-2">
                    <div className="w-6 h-6 rounded-full border bg-white border-gray-300 flex items-center justify-center text-[#0192ED] font-[600] text-[10px]">
                    ...
                    <div className="flex justify-between items-center w-full">
                      ...
                      <p className="text-[#0192ED] font-[600] text-[10px] truncate">{row.user?.userId}</p>
                    </div>
                  </div>
                </div>
              </div>
