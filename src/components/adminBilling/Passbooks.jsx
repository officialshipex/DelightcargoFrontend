    ...
    <div className="space-y-2">
          ...
          <div className="relative">
            <button
              ...
              className={`py-2 px-3 h-9 text-[12px] border rounded-lg font-[600] flex items-center gap-1 transition ${selectedTransactions.length === 0
                ...
                : "text-[#0192ED] border-[#0192ED] bg-white hover:bg-blue-50 shadow-sm"
                }`}
            >
              <span>Actions</span>
            ...
            {desktopDropdownOpen && (
              <div className="absolute right-0 mt-1 w-40 bg-white border-2 border-gray-200 rounded-lg shadow-xl z-[100] animate-popup-in overflow-hidden">
                <ul className="font-[600] text-[12px]">
                  <li
                    className="px-4 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer transition border-b border-gray-50"
                    onClick={handleExport}
                  >
                    Export Excel
                  </li>
                  <li
                    className="px-4 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer transition"
                    onClick={() => {
                      setShowForm(true);
                      setDesktopDropdownOpen(false);
                    }}
      ...
      <div className="hidden md:block relative">
        <div className="h-[calc(100vh-235px)] overflow-y-auto bg-white shadow-sm">
          <table className="min-w-full border-collapse text-[12px] text-left relative">
            <thead className="sticky top-0 z-40 bg-[#0192ED] text-white font-[600]">
              <tr>
                <th className="py-2 px-3 text-left shadow-[0_1px_0_0_#0192ED]">
                ...
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">User Details</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Date</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Order ID</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">AWB Number</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Category</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Amount</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Available Balance</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED] w-[20%] text-left">Description</th>
                <th className="py-2 px-3 shadow-[0_1px_0_0_#0192ED]">Action</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                transactions.map((row, index) => (
                  <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 text-gray-600 transition-colors">
                    ...
                    <td className="py-2 px-3">
                      <p className="text-[#0192ED]">{row.user.userId}</p>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-1 group">
                        <Link to={`/dashboard/order/neworder/updateOrder/${row.orderId}`} className="text-[#0192ED] hover:underline block">
                          {row.orderId}
                        </Link>
                        <button onClick={() => handleCopy(row.orderId, row.id + '_orderId')}>
                          {copiedId === row.id + '_orderId' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400 transition-opacity opacity-0 group-hover:opacity-100" />}
                        </button>
                      </div>
                    </td>
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-1 group">
                        <p className="text-[#0192ED] cursor-pointer hover:underline" onClick={() => handleTrackingByAwb(row.awb_number)}>
                          {row.awb_number}
                        </p>
                        <button onClick={() => handleCopy(row.awb_number, row.id + '_awb')}>
                          {copiedId === row.id + '_awb' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400 transition-opacity opacity-0 group-hover:opacity-100" />}
                        </button>
                      </div>
                    </td>
                    <td className="py-2 px-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] ${row.category === "debit" ? "bg-red-100 text-red-600" : "bg-blue-100 text-[#0192ED]"}`}>
                        {row.category}
                      </span>
                    </td>
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-1 justify-start">
                        <span className={`text-[12px] ${row.category === "debit" ? "text-red-500" : "text-[#0192ED]"} font-medium`}>
                        ...
                        {row.category === "debit" && (
                          ...
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-[#0192ED]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <td className="py-2 px-3">
                      {row.category === "debit" && (
                        <button
                          className="bg-[#0192ED]/10 text-[#0192ED] p-2 rounded-full hover:bg-[#0192ED] hover:text-white transition-all shadow-sm relative group"
            ...
            {mobileDropdownOpen && (
              <div className="absolute right-0 mt-1 w-40 bg-white border-2 border-gray-200 rounded-lg shadow-xl z-[100] animate-popup-in overflow-hidden">
                <ul className="font-[600] text-[11px]">
                  <li className="px-4 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer border-b border-gray-50" onClick={handleExport}>
                    Export Excel
                  </li>
                  <li className="px-4 py-2 text-gray-700 hover:bg-blue-50 cursor-pointer" onClick={() => {
                    setShowForm(true);
                    setMobileDropdownOpen(false);
                  }}>
                    Update Passbook
              ...
              <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-sm p-2 border border-gray-100 text-[10px] animate-popup-in">
                ...
                <div className="flex gap-2 justify-between rounded-lg bg-blue-50 py-1.5 px-2 items-center mb-1.5">
                  ...
                  <div className="text-right">
                    <span className={`px-2 py-0.5 rounded text-[9px] ${row.category === "debit" ? "bg-red-100 text-red-600" : "bg-blue-100 text-[#0192ED]"}`}>
                      {row.category}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-1 mb-1 p-0.5">
                  <div>
                    ...
                    <div className="flex items-center gap-1">
                      <Link to={`/dashboard/order/neworder/updateOrder/${row.orderId}`} className="text-[#0192ED] font-[600] hover:text-[#0192ED] text-[10px]">
                        {row.orderId}
                      </Link>
                      <button onClick={() => handleCopy(row.orderId, row.id + '_orderId_mobile')}>
                        {copiedId === row.id + '_orderId_mobile' ? <FiCheck className="w-2.5 h-2.5 text-[#0192ED]" /> : <FiCopy className="w-2.5 h-2.5 text-gray-400" />}
                      </button>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <p className="text-[#0192ED] font-[600] truncate hover:underline text-[10px]" onClick={() => handleTrackingByAwb(row.awb_number)}>
                        {row.awb_number || "N/A"}
                      </p>
                      {row.awb_number && (
                        <button onClick={() => handleCopy(row.awb_number, row.id + '_awb_mobile')}>
                          {copiedId === row.id + '_awb_mobile' ? <FiCheck className="w-2.5 h-2.5 text-[#0192ED]" /> : <FiCopy className="w-2.5 h-2.5 text-gray-400" />}
                        </button>
                      )}
                    </div>
                  </div>
                  <div className="border-t border-gray-50 pt-1 mt-1 col-span-2 flex justify-between items-center">
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        <p className={`font-[600] text-[10px] ${row.category === "debit" ? "text-red-500" : "text-[#0192ED]"}`}>
                        ...
                        {row.category === "debit" && (
                          <button
                            ...
                            className="text-[#0192ED] p-1.5 -m-1.5"
                          >
                ...
                <div className="mt-1 p-1.5 bg-gray-50 rounded-lg flex justify-between items-center group">
                  ...
                  {row.category === "debit" && (
                    <button
                      onClick={() => handleReverse(row)}
                      className="bg-white text-[#0192ED] p-1.5 rounded-full shadow-sm border border-blue-100 shrink-0"
                    >
                      <FaUndo size={10} />
                    </button>
                  )}
                ...
                <div className="mt-2 flex items-center justify-between px-0.5">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-6 h-6 rounded-full bg-white border border-[#0192ED]/20 flex items-center justify-center font-bold text-[#0192ED] text-[10px] shrink-0 shadow-sm">
                  ...
                  {/* Right Side (User ID) */}
                  <p className="text-[#0192ED] font-bold text-[10px] tracking-widest shrink-0">
                    {row.user?.userId}
                  </p>
      ...
      <div className="fixed sm:bottom-10 bottom-8 right-6 sm:right-4 z-50">
        <button
          ...
          className="bg-[#0192ED] text-white w-10 h-10 rounded-full flex items-center justify-center shadow-2xl hover:opacity-90 transition-all hover:scale-110 active:scale-95 group"
        >
          <FaBook size={16} />
