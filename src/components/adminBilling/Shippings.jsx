        ...
        <div className="h-[calc(100vh-235px)] text-[12px] overflow-y-auto bg-white">
          <table className="w-full border-collapse">
            <thead className="bg-[#0192ED] text-white font-[600] sticky top-0 z-10">
              <tr className="bg-[#0192ED] text-white font-[600] text-left">
              ...
              {loading ? (
                ...
                transactions.map((row, index) => (
                  <tr key={index} className="border-b border-gray-300 font-[400] text-[12px] hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">
                      <p className="text-[#0192ED] font-medium">{row.user?.userId}</p>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1 group">
                        <Link to={`/dashboard/order/neworder/updateOrder/${row._id}`} className="text-[#0192ED] font-medium">
                          {row.orderId}
                        </Link>
                        <button onClick={() => handleCopy(row.orderId, row._id + '_id')}>
                          {copiedId === row._id + '_id' ? <FiCheck className="text-blue-500" /> : <FiCopy className="text-gray-400 opacity-0 group-hover:opacity-100" />}
                        </button>
                      </div>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1 group">
                        <p className="text-[#0192ED] cursor-pointer" onClick={() => handleTrackingByAwb(row.awb_number)}>
                          {row.awb_number || "AWB Pending"}
                        </p>
                        {row.awb_number && (
                          <button onClick={() => handleCopy(row.awb_number, row._id + '_awb')}>
                            {copiedId === row._id + '_awb' ? <FiCheck className="text-blue-500" /> : <FiCopy className="text-gray-400 opacity-0 group-hover:opacity-100" />}
                          </button>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3">
                      <span className="text-[#0192ED] bg-blue-100 rounded px-2 py-0.5 text-[10px]">{row.status}</span>
                    </td>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1">
                        ...
                        {(row.totalFreightCharges || row.weightDiscrepancy?.excessWeightCharges?.excessCharges) && (
                          ...
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-[#0192ED]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" strokeWidth="2" /><path d="M12 8v4m0 4h.01" strokeWidth="2" strokeLinecap="round" /></svg>
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3 text-[#0192ED]">
                      <button
            ...
            transactions.map((row) => (
              <div key={row._id} className="bg-white rounded-lg shadow-sm px-3 py-2 border border-gray-200 animate-popup-in relative">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-gray-700 font-[600] text-[10px]">Order ID : <span className="text-[#0192ED] font-[600]">{row.orderId}</span></span>
                    ...
                    <div className="flex items-center gap-1 group">
                      ...
                      {row.awb_number && (
                        <button onClick={() => handleCopy(row.awb_number, row._id + '_awb_mob')}>
                          {copiedId === row._id + '_awb_mob' ? <FiCheck className="text-blue-500" /> : <FiCopy className="text-gray-300" />}
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  {mobilePricePopupId === row._id && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                      ...
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0, scale: 0.9 }}
                        className="bg-white rounded-lg shadow-xl w-full max-w-xs relative z-10 flex flex-col max-h-[85vh] overflow-hidden"
                      >
                        ...
                        {row.orderType === "B2C" ? (
                          <div className="space-y-2 text-[12px] px-4 pb-4">
                          ...
                          <div className="text-[12px] flex flex-col min-h-0 flex-1">
                            <B2BRateBreakup rateBreakup={row.rateBreakup} total={row.totalFreightCharges} padX="px-4" />
                          </div>
                        )}
                      </motion.div>
                    </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center text-[#0192ED] font-bold text-[12px]">
      ...
      {pricePopupPos && (
        ...
        >
          ...
          {pricePopupPos.order.orderType === "B2C" ? (
            ...
            <B2BRateBreakup rateBreakup={pricePopupPos.order.rateBreakup} total={pricePopupPos.order.totalFreightCharges} />
          )}
        </div>
      )}
    </div>
