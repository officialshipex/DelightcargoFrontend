...
const AdminCodRemittances = ({ isSidebarAdmin }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isSidebarAdmin) {
          ...
          const canView = !!employee?.accessRights?.finance?.["COD Remittance Order"]?.view;
          setEmployeeAccess({ canView });
          if (!canView) return;
        }

        let url = `${REACT_APP_BACKEND_URL}/cod/CodRemittanceOrder?page=${page}&limit=${limit}`;
        ...
        if (response.status === 200) {
          ...
          if (response.data.data.orders.length > 0) {
            const couriers = Array.from(new Set(response.data.data.orders.map(o => o.courierProvider).filter(Boolean)));
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(remitted.map(r => r._id || r.orderID));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleExport = () => {
    ...
    const dataToExport = remitted.filter(r => selectedIds.includes(r._id || r.orderID));
    const formattedData = dataToExport.map((row) => ({
      ...
      "AWB Number": row.AWB_Number,
      "Courier Name": row.courierProvider,
      "COD Amount": row.CODAmount || 0,
      Status: row.status,
    }));
    const worksheet = XLSX.utils.json_to_sheet(formattedData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "COD Orders");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([excelBuffer], { type: "application/octet-stream" }), `cod_orders_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const fieldStyle = "w-full h-[36px] px-3 text-[12px] font-[600] border border-gray-200 rounded-lg focus:outline-none focus:border-[#0192ED] transition-all bg-white";
    ...
    <div className="space-y-2">
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-2 my-2">
          {summaryItems.map((item, idx) => (
            <div key={idx} className="flex items-center gap-4 p-2 bg-white rounded-lg border border-[#0192ED] hover:shadow-sm transition-shadow">
              <div className="bg-[#0192ED] text-white p-2 rounded-full">{item.icon}</div>
      ...
      <div className="hidden md:flex flex-row gap-2 items-center z-20">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button onClick={() => setUpload(true)} className="h-9 px-3 rounded-lg text-[12px] font-bold bg-[#0192ED] text-white hover:bg-opacity-90 shadow-sm transition-all focus:scale-95 flex items-center justify-center gap-1">
          ...
          <button
            ...
            className={`h-9 px-4 rounded-lg text-[12px] font-bold flex items-center justify-center gap-1 border transition-all ${selectedIds.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50 shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"}`}
          >
      ...
      <div className="md:hidden flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2.5 bg-[#0192ED] text-white rounded-lg active:scale-95 transition-transform"
          >
        ...
        <div className="flex items-center justify-between bg-white p-2 gap-2 rounded-lg border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-lg w-full">
            <input
              type="checkbox"
              checked={remitted.length > 0 && selectedIds.length === remitted.length}
          ...
          <div className="relative" ref={bulkActionRef}>
            <button
              ...
              className={`py-[8px] px-3 rounded-lg border transition-all ${selectedIds.length > 0 ? "border-[#0192ED] text-[#0192ED] shadow-sm" : "border-gray-200 text-gray-400 bg-gray-50 cursor-not-allowed"}`}
              disabled={selectedIds.length === 0}
            >
            ...
            {bulkActionOpen && (
              <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-xl shadow-2xl border border-gray-100 z-[100] overflow-hidden animate-popup-in">
                <button
                  ...
                  className="w-full px-3 py-2 text-[12px] font-bold text-gray-700 hover:bg-blue-50 hover:text-[#0192ED] flex items-center gap-2.5 transition-colors border-b border-gray-50"
                >
                  <Download className="w-3 h-3 text-[#0192ED]" />
                  Export
                </button>
              </div>
            )}
      ...
      <div className="hidden md:block">
        <div className="h-[calc(100vh-300px)] overflow-y-auto bg-white overflow-hidden shadow-sm shadow-green-50/50">
          <table className="w-full text-[12px] border-collapse sticky-header">
            <thead className="bg-[#0192ED] text-white font-bold sticky top-0 z-10 text-left">
              <tr>
                <th className="py-2 px-3">
                  <div className="flex justify-center items-center">
                    <input type="checkbox" checked={remitted.length > 0 && selectedIds.length === remitted.length} onChange={handleSelectAll} className="cursor-pointer accent-[#0192ED] w-3 h-3" />
                  </div>
                </th>
            ...
            <tbody>
              {loading ? (
                ...
                remitted.map((row, index) => {
                  ...
                  return (
                    <tr key={index} className="border-t border-gray-200 hover:bg-blue-50/30 transition-colors">
                      <td className="py-2 px-3">
                        <div className="flex justify-center items-center">
                          <input type="checkbox" checked={selectedIds.includes(rowId)} onChange={() => handleSelectRow(rowId)} className="cursor-pointer accent-[#0192ED] w-3 h-3" />
                        </div>
                      </td>
                      <td className="py-2 px-3">
                        <p className="text-blue-500 font-bold">{row.userId}</p>
                        <p className="text-gray-700">{row.userName}</p>
                        <p className="text-gray-700">{row.Email}</p>
                      </td>
                      <td className="py-2 px-3">
                        <div className="flex items-center gap-2 group">
                          <Link to={`/dashboard/order/neworder/updateOrder/${row.orderID}`} className="text-[#0192ED] font-bold hover:underline transition-all">
                            {row.orderID}
                          </Link>
                          <button onClick={() => handleCopy(row.orderID, row.orderID)} className="p-1 hover:bg-blue-100 rounded text-gray-400 hover:text-[#0192ED] transition-all opacity-0 group-hover:opacity-100">
                            {copiedId === row.orderID ? <FiCheck className="text-blue-500 w-3 h-3" /> : <FiCopy className="w-3 h-3" />}
                          </button>
                        </div>
                      </td>
                      <td className="py-2 px-3 leading-tight">
                        <div className="flex items-center gap-2 group">
                          <Link to={`/dashboard/order/tracking/${row.AWB_Number}`} className="text-[#0192ED] font-bold flex items-center gap-1 hover:underline">
                            {row.AWB_Number}

                          </Link>
                          <button onClick={() => handleCopy(row.AWB_Number, row.AWB_Number + '_awb')} className="p-1 hover:bg-blue-100 rounded text-gray-400 hover:text-[#0192ED] transition-all opacity-0 group-hover:opacity-100">
                            {copiedId === row.AWB_Number + '_awb' ? <FiCheck className="text-blue-500 w-3 h-3" /> : <FiCopy className="w-3 h-3" />}
                          </button>
                        </div>
                        <div className="flex flex-col">
                          <p className="text-gray-700 text-[12px]">{row.courierProvider}</p>
                      ...
                      <td className="py-2 px-3">
                        <span className={`px-2 py-0.5 rounded text-[10px] shadow-sm ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>{row.status}</span>
                      </td>
                    </tr>
                  )
                })
      ...
      <div className="md:hidden flex flex-col space-y-2 h-[calc(100vh-320px)] overflow-y-auto">
        {loading ? (
          ...
          <div className="flex justify-center items-center py-20 flex-col">
            ...
            {/* <p className="text-gray-400 font-bold mt-4 font-[600]">No Remittances Found</p> */}
          </div>
        ) : (
          remitted.map((row, index) => {
            const rowId = row._id || row.orderID;
                ...
                <div className="flex gap-2 justify-between rounded-lg bg-blue-50 py-1.5 px-2 items-center mb-1.5">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-3 h-3 accent-[#0192ED] cursor-pointer"
                    />
                    <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center p-0.5 border shadow-xs overflow-hidden shrink-0">
                      <img
                        src={getCarrierLogo(row?.courierProvider)}
                        alt=""
                  ...
                  <div className="text-right">
                    <span className={`px-2 py-0.5 rounded text-[10px] ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>
                      {row.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-1 p-0.5 items-center">
                  <div>
                    ...
                    <div className="flex items-center gap-1">
                      <Link to={`/dashboard/order/neworder/updateOrder/${row.orderID}`} className="text-[#0192ED] font-[600] text-[10px] hover:underline truncate max-w-[60px]">
                        #{row.orderID}
                      </Link>
                      <button onClick={() => handleCopy(row.orderID, row.orderID + '_m')}>
                        {copiedId === row.orderID + '_m' ? <FiCheck className="w-2 h-2 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400" />}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <Link to={`/dashboard/order/tracking/${row.AWB_Number}`} className="text-[#0192ED] font-[600] truncate hover:underline text-[10px] max-w-[70px]">
                        {row.AWB_Number || "N/A"}
                      </Link>
                      <button onClick={() => handleCopy(row.AWB_Number, row.AWB_Number + '_m_m')}>
                        {copiedId === row.AWB_Number + '_m_m' ? <FiCheck className="w-2 h-2 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400" />}
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {upload && <CodUploadPoopup onClose={() => setUpload(false)} setRefresh={setRefresh} />}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full h-9 px-3 font-[600] text-[12px] border border-gray-300 rounded-lg focus:outline-none focus:border-[#0192ED] transition-all text-gray-700 placeholder:text-gray-400"
                ...
                <div className="space-y-1.5" ref={statusRef}>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`w-full h-9 px-3 text-[12px] font-[600] border rounded-lg focus:outline-none flex items-center justify-between transition-all bg-white text-left ${statusDropdownOpen ? "border-[#0192ED]" : "border-gray-300"}`}
                    >
                        ...
                        >
                          {["Paid", "Pending"].map(s => (
                            <div
                              ...
                              className={`px-3 py-2 text-[12px] font-[600] cursor-pointer transition-colors ${localFilters.status === s ? "bg-blue-50 text-[#0192ED]" : "text-gray-500 hover:bg-gray-50 hover:text-[#0192ED]"}`}
                            >
                              {s}
                            </div>
                          ))}
                ...
                <div className="space-y-1.5" ref={courierRef}>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`w-full h-9 px-3 text-[12px] font-[600] border rounded-lg focus:outline-none flex items-center justify-between transition-all bg-white text-left ${courierDropdownOpen ? "border-[#0192ED]" : "border-gray-300"}`}
                    >
                        ...
                        >
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full h-8 pl-8 pr-3 text-[11px] font-[500] border border-gray-200 rounded-md focus:border-[#0192ED] focus:outline-none transition-all"
                            />
                          </div>
                          ...
                          <div className="max-h-[160px] overflow-y-auto pr-1 space-y-1 custom-scrollbar-thin">
                            {filteredCourierOptions.length > 0 ? filteredCourierOptions.map(c => (
                              <label key={c} className={`flex items-center gap-2.5 p-1.5 rounded-md cursor-pointer transition-all ${localFilters.couriers.includes(c) ? "bg-blue-50 text-[#0192ED]" : "hover:bg-gray-50 text-gray-600"}`}>
                                <input
                                  type="checkbox"
                                  checked={localFilters.couriers.includes(c)}
                                  onChange={() => toggleCourier(c)}
                                  className="w-3.5 h-3.5 accent-[#0192ED] rounded border-gray-300"
                                />
              ...
              <div className="p-4 border-t border-gray-100 bg-white flex gap-3 mt-auto">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#0192ED] text-white rounded-lg text-[12px] font-[600] hover:bg-blue-600 transition-all shadow-md active:scale-95"
                >
                  Apply Filters
                </button>
              </div>
