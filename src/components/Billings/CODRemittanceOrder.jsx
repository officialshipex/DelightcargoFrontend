  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-[12px] font-bold">
        <div className="md:hidden border text-[10px] border-[#0192ED] bg-white rounded-lg px-3 py-2 space-y-2">
        ...
        <div className="hidden md:grid md:grid-cols-5 gap-2 my-2">
          {summaryItems.map((item, idx) => (
            <div key={idx} className="flex items-center gap-4 p-2 bg-white rounded-lg border border-[#0192ED] hover:shadow-sm transition-shadow">
              <div className="bg-[#0192ED] text-white p-2 rounded-full shrink-0">{item.icon}</div>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <div className="relative" ref={bulkActionDesktopRef}>
            <button
              ...
              className={`h-9 px-4 rounded-lg text-[12px] font-bold flex items-center gap-1 border transition-all ${selectedRemittanceIds.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50 shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"}`}
            >
              Actions
            ...
            {bulkActionOpen && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-100 rounded-lg shadow-xl w-52 text-[12px] z-[100] animate-popup-in overflow-hidden border">
                <div className="px-4 py-2 hover:bg-blue-50 cursor-pointer font-bold text-gray-700 flex items-center gap-2 border-b border-gray-50 transition-colors" onClick={() => { handleExport(); setBulkActionOpen(false); }}>
                  <Download className="w-4 h-4 text-[#0192ED]" /> Export Data
                </div>
                <div
                  className={`px-4 py-2 hover:bg-blue-50 cursor-pointer font-bold text-gray-700 flex items-center gap-2 border-b border-gray-50 transition-colors ${bankExportLoading ? "opacity-60 cursor-not-allowed" : ""}`}
                ...
                <div className="px-4 py-2 hover:bg-blue-50 cursor-pointer font-bold text-gray-700 flex items-center gap-2 border-b border-gray-50 transition-colors" onClick={() => { handleOpenBankResponseUpload(); setBulkActionOpen(false); }}>
                  <Upload className="w-4 h-4 text-orange-500" /> Upload Bank Response
                </div>
                <div className="px-4 py-2 hover:bg-blue-50 cursor-pointer font-bold text-gray-700 flex items-center gap-2 transition-colors" onClick={() => { handleTransferCOD(); setBulkActionOpen(false); }}>
                  <Send className="w-4 h-4 text-[#0192ED]" /> Transfer COD
                </div>
              </div>
            )}
          </div>
      ...
      <div className="md:hidden flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          {/* <button
            ...
            className={`p-2.5 rounded-lg active:scale-95 transition-transform ${employeeAccess.canAction || employeeAccess.isAdmin ? "bg-[#0192ED] text-white" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
          >
        ...
        <div className="flex items-center justify-between bg-white p-2 gap-2 rounded-lg border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-lg w-full">
            <input
              ...
              className="w-3 h-3 accent-[#0192ED] cursor-pointer shadow-sm"
            />
          ...
          <div className="relative" ref={bulkActionMobileRef}>
            <button
              onClick={() => setBulkActionOpen(!bulkActionOpen)}
              className={`py-[8px] px-3 rounded-lg border transition-all ${selectedRemittanceIds.length > 0 ? "border-[#0192ED] text-[#0192ED] shadow-sm" : "border-gray-200 text-gray-400 bg-gray-50 cursor-not-allowed"}`}
              disabled={selectedRemittanceIds.length === 0}
            >
            ...
            {bulkActionOpen && (
              <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-2xl border border-gray-100 z-[100] overflow-hidden animate-popup-in">
                <button
                  ...
                  className="w-full px-4 py-2 text-[12px] font-bold text-gray-700 hover:bg-blue-50 hover:text-[#0192ED] flex items-center gap-2.5 transition-colors border-b border-gray-50"
                >
                  <Download className="w-3 h-3 text-[#0192ED]" /> Export Data
                </button>
                <button
                  onClick={() => { if (!bankExportLoading) { handleExportBankTemplate(); setBulkActionOpen(false); } }}
                ...
                <button
                  ...
                  className="w-full px-4 py-2 text-[12px] font-bold text-gray-700 hover:bg-blue-50 hover:text-[#0192ED] flex items-center gap-2.5 transition-colors"
                >
                  <Send className="w-3 h-3 text-[#0192ED]" /> Transfer COD
                </button>
              </div>
            )}
          </div>
      ...
      <div className="hidden md:block">
        <div className="h-[calc(100vh-300px)] overflow-y-auto bg-white overflow-hidden shadow-sm shadow-green-50/50">
          <table className="w-full text-[12px] border-collapse sticky-header">
            <thead className="bg-[#0192ED] text-white font-bold sticky top-0 z-10 text-center tracking-wider">
              <tr>
                <th className="py-2 px-3">
                  <div className="flex justify-center items-center">
                    <input type="checkbox" checked={remitedData.length > 0 && selectedRemittanceIds.length === remitedData.length} onChange={handleSelectAll} className="accent-[#0192ED] cursor-pointer w-3 h-3" />
                  </div>
                </th>
            ...
            <tbody className="text-center">
              {loading ? (
                ...
                remitedData.map((row, index) => (
                  <tr key={index} className="border-t border-gray-100 hover:bg-blue-50/20 transition-colors">
                    <td className="py-2 px-3">
                      <div className="flex justify-center items-center">
                        <input type="checkbox" checked={selectedRemittanceIds.includes(row.remittanceId)} onChange={() => handleCheckboxChange(row.remittanceId)} className="accent-[#0192ED] cursor-pointer w-3 h-3" />
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center justify-center gap-2 group">
                        <button onClick={() => openRemittanceDetails(row.remittanceId)} className="text-[#0192ED] font-bold hover:underline transition-all">
                          {row.remittanceId}
                        </button>
                        <button onClick={() => handleCopy(row.remittanceId, row.remittanceId)} className="p-1 hover:bg-blue-100 rounded text-gray-400 hover:text-[#0192ED] transition-all opacity-0 group-hover:opacity-100">
                          {copiedId === row.remittanceId ? <FiCheck className="text-blue-500 w-3 h-3" /> : <FiCopy className="w-3 h-3" />}
                        </button>
                      </div>
                    </td>
                    <td className="py-2 px-3 text-[#0192ED] font-[600]">
                      <div className="flex items-center justify-center gap-1 group">
                        <span>{row.utr || "N/A"}</span>
                        {row.utr && (
                          <button onClick={() => handleCopy(row.utr, row.remittanceId + '_utr')} className="p-1 hover:bg-blue-100 rounded text-gray-400 hover:text-[#0192ED] transition-all opacity-0 group-hover:opacity-100">
                            {copiedId === row.remittanceId + '_utr' ? <FiCheck className="text-blue-500 w-3 h-3" /> : <FiCopy className="w-3 h-3" />}
                          </button>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3 text-[#0192ED] font-[600]">₹{(Number(row.remittanceInitiated) || 0).toFixed(2)}</td>
                    <td className="py-2 px-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] shadow-sm ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "bg-orange-100 text-orange-600"}`}>{row.status}</span>
                    </td>
                  </tr>
                ))
              )}
        ...
        {loading ? (
          ...
          remitedData.map((row, index) => (
            <div key={index} className={`bg-white border border-gray-200 rounded-lg shadow-sm p-2 text-[10px] animate-popup-in relative ${detailsPopupId === row.remittanceId ? 'z-50' : 'z-10'}`}>
              ...
              <div className="flex gap-2 justify-between rounded-lg bg-blue-50 py-1.5 px-2 items-center mb-1.5">
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={selectedRemittanceIds.includes(row.remittanceId)}
                    onChange={() => handleCheckboxChange(row.remittanceId)}
                    className="w-3.5 h-3.5 accent-[#0192ED]"
                  />
                  <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center p-0.5 border shadow-xs overflow-hidden shrink-0">
                    <Wallet className="w-3.5 h-3.5 text-[#0192ED]" />
                  </div>
                </div>
                ...
                <div className="text-right flex flex-col items-end">
                  <span className={`px-2 py-0.5 rounded text-[10px] shadow-xs ${row.status === "Paid" ? "bg-blue-100 text-[#0192ED]" : "bg-red-100 text-red-600"}`}>
                    {row.status}
                  </span>
                  <p
                    className="text-[9px] text-[#0192ED] font-bold border-b border-dashed border-[#0192ED] cursor-pointer mt-1"
              ...
              <div className="grid grid-cols-3 gap-2 mb-1 p-0.5 items-center">
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="text-[#0192ED] font-[600] text-[10px] hover:underline truncate max-w-[60px]"
                    >
                      {row.remittanceId}
                    </button>
                    <button onClick={() => handleCopy(row.remittanceId, row.remittanceId + '_m')}>
                      {copiedId === row.remittanceId + '_m' ? (
                        <FiCheck className="w-2 h-2 text-[#0192ED]" />
                      ) : (
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center justify-end gap-1">
                    <span className="text-[#0192ED] font-[600] truncate text-[10px] max-w-[70px]">
                      {row.utr || "N/A"}
                    </span>
                    {row.utr && (
              ...
              <div className="mt-2 flex items-center justify-between px-0.5 border-t border-gray-100 pt-2">
                <div className="flex items-center gap-2 min-w-0 max-w-[70%]">
                  <div className="w-7 h-7 rounded-full bg-[#0192ED]/10 border border-[#0192ED]/20 flex items-center justify-center font-bold text-[#0192ED] text-[10px] shrink-0 shadow-sm uppercase">
                  ...
                  <>
                    ...
                    >
                      ...
                      <div className="space-y-2 text-[10px]">
                        ...
                        <div className="flex justify-between items-center bg-blue-50/50 p-2 rounded-lg border border-blue-100 mt-1">
                          <span className="text-[#0192ED] font-bold">Net Payout</span>
                          <span className="text-[#0192ED] font-[600] text-[10px]">₹{(Number(row.remittanceInitiated) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                      </div>
                    </motion.div>
                  </>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-blue-50 rounded-xl border border-blue-100">
                  <FileSpreadsheet className="w-6 h-6 text-[#0192ED]" />
                </div>
                <div>
                ...
                {selectedBankFile ? (
                  <div className="flex flex-col items-center">
                    <div className="p-2 bg-blue-100 rounded-full mb-2">
                      <CheckCircle className="w-8 h-8 text-[#0192ED]" />
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-1.5 bg-[#0192ED] text-white rounded-lg text-[10px] sm:text-[11px] font-bold hover:bg-blue-600 transition flex items-center gap-1.5 shadow-sm"
                      >
                  ...
                  <div>
                    ...
                    <label
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-[11px] font-bold cursor-pointer transition-all bg-[#0192ED] text-white hover:bg-blue-600 shadow-sm"
                    >
          ...
          <div className="fixed inset-0 z-[110] flex justify-end">
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "0%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="relative w-full max-w-[340px] bg-white h-full shadow-2xl flex flex-col border-l border-gray-200">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-9 pl-3 pr-3 text-[12px] font-[600] border border-gray-300 rounded-lg focus:outline-none focus:border-[#0192ED] transition-all text-gray-700 placeholder:text-gray-400"
                ...
                <div className="space-y-1.5" ref={statusRef}>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`w-full h-9 px-3 text-[12px] font-[600] border rounded-lg focus:outline-none flex items-center justify-between transition-all bg-white text-left ${statusDropdownOpen ? "border-[#0192ED]" : "border-gray-200"}`}
                    >
                    ...
                    {statusDropdownOpen && (
                      <div className="absolute top-[105%] left-0 w-full bg-white border border-gray-100 rounded-lg shadow-xl z-[120] py-1 border border-gray-200 animate-popup-in">
                        {["Paid", "Pending"].map(s => (
                          <div key={s} onClick={() => { setLocalFilters(prev => ({ ...prev, status: s })); setStatusDropdownOpen(false); }} className={`px-4 py-2 text-[12px] font-[600] cursor-pointer transition-colors ${localFilters.status === s ? "bg-blue-50 text-[#0192ED]" : "text-gray-500 hover:bg-gray-50"}`}>{s}</div>
                        ))}
                      </div>
                    )}
                  </div>
              ...
              <div className="p-4 border-t bg-gray-50 flex gap-3 mt-auto">
                ...
                <button onClick={() => { setAppliedFilters(localFilters); setIsFilterPanelOpen(false); setPage(1); }} className="flex-1 py-2 bg-[#0192ED] text-white rounded-lg text-[12px] font-bold hover:opacity-90 transition-all shadow-md">Apply Filter</button>
              </div>
            </motion.div>
          </div>
        )}
