...
const SellerRemittanceDatas = ({ remittanceId }) => {
    ...
    const fetchRemittanceData = async () => {
      try {
        ...
        const response = await axios.get(
          `${REACT_APP_BACKEND_URL}/cod/sellerremittanceTransactionData/${id}`,
          {
        ...
        if (response.data.success && response.data.data) {
          ...
          console.error("API Error: Unsuccessful response", response.data);
        }
      } catch (error) {
  ...
  const navigateToBilling = () => navigate("/finance/COD/sellerCodRemittance");
  ...
  const StatusBadge = ({ status }) => {
    if (status === "Paid") {
      return (
        <div className="flex items-center gap-1 text-[#0192ED] font-[600] sm:text-[14px] text-[12px]">
  ...
  return (
    <div className="px-3 py-2 max-w-4xl">
      ...
      <div className="flex w-full mb-2 justify-between items-center">
        ...
        <div className="flex flex-col">
          <h2 className="text-[12px] sm:text-[14px] font-[600] text-gray-700">
            Remittance ID : <span className="text-[#0192ED]">{remittanceId || "N/A"}</span>
          </h2>
      ...
      <div className="overflow-x-auto max-h-[350px] overflow-y-auto mb-2">
        ...
        <table className="sm:w-full min-w-[700px] text-left">
          <thead className="bg-[#0192ED] text-white sticky top-0 z-10">
            <tr className="text-[12px] font-[600] border border-[#0192ED]">
                ...
                >
                  ...
                  <td className="px-3 py-2 text-[#0192ED]">
                    <button
                      ...
                      className="text-[#0192ED] hover:underline"
                    >
                      {item.awb_number}
                    </button>
                  </td>
      ...
      <div className="w-full flex justify-center mt-3">
        <button
          ...
          className="bg-[#0192ED] text-white px-3 py-2 rounded-lg text-[12px] font-[600] hover:bg-blue-500 transition"
        >
          Export
        </button>
      </div>
...
export default SellerRemittanceDatas;
