        ...
        <div className="flex items-center gap-2 ml-auto" ref={actionRef}>
          ...
          <div className="relative">
            <button
              ...
              className={`h-9 px-3 rounded-lg text-[12px] font-[600] flex items-center gap-1 border transition-all ${selectedTransactions.length > 0 ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50 shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
              Actions
                  ...
                  <tr key={row._id} className="hover:bg-gray-50 text-[12px] transition-colors border-b border-gray-200 text-gray-600">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1 group">
                        <span className="text-[#0192ED]">{row.paymentDetails?.transactionId}</span>
                    ...
                    <td className="px-3 py-2">
                      <span className={`px-2 py-0.5 rounded text-[10px] ${row.status === "success" ? "bg-blue-50 text-[#0192ED]" : "bg-red-50 text-red-500"}`}>
                        {row.status}
                      </span>
                    </td>
                    <td className="px-3 py-2">
                          ...
                          <div className="flex items-center gap-2 group">
                            ...
                            {row.paymentDetails?.paymentId && (
                              <button onClick={() => handleCopy(row.paymentDetails.paymentId, row._id + '_pay')}>
                                {copiedId === row._id + '_pay' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-400 transition-opacity opacity-0 group-hover:opacity-100" />}
                              </button>
                            )}
                          </div>
                        </div>
              ...
              <div key={row._id} className="bg-white rounded-xl shadow-sm p-3 border border-gray-100 text-[10px] animate-popup-in">
                <div className="flex justify-between mb-2 items-start">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1">
                        <span className="text-gray-700 font-[600] text-[10px]">Transaction Id : <span className="text-[#0192ED] font-[600]">{row.paymentDetails?.transactionId}</span></span>
                        <button onClick={() => handleCopy(row.paymentDetails?.transactionId, row._id + '_txn_mobile')}>
                          {copiedId === row._id + '_txn_mobile' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-300" />}
                        </button>
                      </div>
                  ...
                  <div className="text-right">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] inline-block mt-1 ${row.status === "success" ? "bg-blue-50 text-[#0192ED]" : "bg-red-50 text-red-500"}`}>
                      {row.status}
                    </span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 bg-blue-50/50 p-2 rounded-lg border border-gray-100">
                  <div className="space-y-0.5">
                    <p className="text-gray-700 text-[10px] tracking-wider">Payment ID</p>
                    <div className="flex items-center gap-1">
                      <p className="text-[#0192ED] font-[600] truncate max-w-[120px] text-[10px]">{row.paymentDetails?.paymentId || "N/A"}</p>
                      {row.paymentDetails?.paymentId && (
                        <button onClick={() => handleCopy(row.paymentDetails.paymentId, row._id + '_pay_mobile')}>
                          {copiedId === row._id + '_pay_mobile' ? <FiCheck className="w-3 h-3 text-[#0192ED]" /> : <FiCopy className="w-3 h-3 text-gray-300" />}
                        </button>
                      )}
                    </div>
                  </div>
                  {row.paymentDetails?.orderId && (
                    <div className="text-right space-y-0.5">
                      ...
                      <p className="text-[#0192ED] font-[600] truncate text-[10px]">{row.paymentDetails.orderId}</p>
                    </div>
                  )}
                </div>
              </div>
