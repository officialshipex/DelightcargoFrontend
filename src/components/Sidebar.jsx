...
const SidebarItem = ({
  ...
  return (
    <div className="transition-all duration-300 ease-in-out">
      ...
      ${isActive ? "bg-[#0192ED] text-white" : "text-gray-500 hover:bg-[#0192ED]"} 
      ...
      {extent && (
        ...
        >
          {list.map((item, index) => (
            <li
              key={index}
              className="cursor-pointer hover:text-white hover:bg-[#0192ED] transition-colors duration-300 p-1"
              onClick={() => {
...
const Sidebar = ({ isAdmin: isAdminProp, adminTab: adminTabProp }) => {
  ...
  return (
    <>
      {isMobile && (
        <button
          className={`fixed left-2 z-50 text-[#0192ED] p-2 transition-all duration-300 ${localStorage.getItem("admin_token_backup") ? "top-[40px]" : "top-[8px]"}`}
