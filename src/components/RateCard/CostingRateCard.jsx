  ...
  const Dropdown = ({ label, value, options, open, setOpen, onSelect }) => (
    <div className="relative flex-1 sm:max-w-[200px]">
      <div
        ...
        className="flex items-center justify-between font-[600] border border-gray-300 bg-white px-3 py-1.5 h-9 rounded-lg cursor-pointer hover:border-[#0192ED] transition-all group"
      >
        <span className={`text-[10px] sm:text-[12px] truncate ${value ? "text-gray-700" : "text-gray-400"}`}>
          {value || label}
        </span>
        <FiChevronDown className={`text-gray-400 group-hover:text-[#0192ED] transition-transform flex-shrink-0 ${open ? "rotate-180" : ""}`} />
      </div>
      {open && (
        <div className="absolute font-[600] top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-[100] max-h-48 overflow-y-auto">
          {options.map((opt, i) => (
            <div
              ...
              className="px-3 py-2 text-[10px] sm:text-[12px] hover:bg-[#0192ED]/10 hover:text-[#0192ED] cursor-pointer transition-colors"
            >
              {opt}
            </div>
          ))}
      ...
      {formData[type].map((row, i) => (
        <div key={i} className="grid grid-cols-3 sm:flex gap-2 mt-2">
          <input
            ...
            value={row.weight}
            className={`border h-9 text-gray-700 font-[600] px-3 rounded-lg text-[10px] sm:text-[12px] w-full focus:border-[#0192ED] focus:outline-none transition-all ${errors[`${type === "weightPriceBasic" ? "basic" : "add"}Weight-${i}`] ? "border-red-400" : "border-gray-300"}`}
          ...
          {ZONES.map((z) => (
            <input
              ...
              placeholder={`Zone ${z} * ₹`}
              value={row[`zone${z}`]}
              className={`border text-gray-700 font-[600] px-3 h-9 rounded-lg text-[10px] sm:text-[12px] w-full focus:border-[#0192ED] focus:outline-none transition-all ${errors[`${type === "weightPriceBasic" ? "basic" : "add"}Zone${z}-${i}`] ? "border-red-400" : "border-gray-300"}`}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3 shadow-sm">
        <div className="flex items-center justify-between mb-3 border-b border-gray-100 pb-2">
          <h2 className="text-[12px] sm:text-[14px] font-bold text-gray-700">
            ...
            <span className="text-[#0192ED]">{editingId ? "Edit" : "Add"}</span>
          </h2>
          {editingId && (
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-2">
          ...
          <div className="relative flex-1 sm:max-w-[240px]">
            <div
              ...
              className={`flex items-center justify-between font-[600] border border-gray-300 bg-white px-3 py-1.5 h-9 rounded-lg transition-all group ${editingId ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-[#0192ED]"}`}
            >
              ...
              <FiChevronDown className={`text-gray-400 group-hover:text-[#0192ED] transition-transform flex-shrink-0 ${isServiceOpen ? "rotate-180" : ""}`} />
            </div>
        ...
        <div className="mt-3">
          ...
          <div className="flex gap-2 mt-1">
            <input
              ...
              className={`border h-9 text-gray-700 font-[600] px-3 rounded-lg text-[10px] sm:text-[12px] w-full sm:w-auto focus:border-[#0192ED] focus:outline-none transition-all ${errors.cod ? "border-red-400" : "border-gray-300"}`}
            />
            <input
              type="number"
              name="codPercent"
        ...
        <div className="mt-4 flex justify-end gap-2 border-t border-gray-100 pt-3">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="bg-[#0192ED] text-white hover:bg-opacity-90 px-4 py-2 rounded-lg text-[10px] sm:text-[12px] font-bold transition-all active:scale-95 disabled:opacity-60"
          >
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100">
          ...
          <div className="relative font-[600] w-[200px]">
            ...
            <input
              ...
              className="w-full border border-gray-300 pl-8 pr-3 py-1.5 h-8 rounded-lg text-[10px] sm:text-[12px] bg-white focus:border-[#0192ED] focus:ring-1 focus:ring-[#0192ED]/20 outline-none transition-all placeholder:text-gray-400"
            />
          </div>
        </div>
        ...
        <div className="hidden sm:block overflow-auto" style={{ height: tableHeight, minHeight: "200px" }}>
          <table className="w-full text-center border-collapse">
            <thead className="sticky top-0 z-20 bg-[#0192ED] text-[12px] text-white">
              <tr>
                      ...
                      <td className="px-3 py-2" rowSpan={2}>
                        <div className="flex justify-center items-center gap-2">
                          <button
                            ...
                            className="text-[#0192ED] hover:scale-110 transition-transform"
                            title="Edit"
                          >
                            <FaEdit size={12} />
                          </button>
              ...
              <div key={idx} className="bg-white rounded-lg border border-gray-200 p-3 text-[10px] space-y-2 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleEdit(card)} className="text-[#0192ED]"><FaEdit size={12} /></button>
                ...
                <div className="grid grid-cols-2 gap-2 bg-gray-50 rounded p-2">
                  <div>
                    ...
                    <span className="font-bold text-[#0192ED]">₹{card.weightPriceBasic?.[0]?.zoneA ?? "—"}</span>
                  </div>
                  <div>
                    <span className="text-gray-500 block">Addl (Zone A)</span>
                    <span className="font-bold text-[#0192ED]">₹{card.weightPriceAdditional?.[0]?.zoneA ?? "—"}</span>
                  </div>
                </div>
