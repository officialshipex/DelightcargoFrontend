  ...
  return (
    <div ref={dropdownRef} className="relative w-full flex flex-col gap-1">
      ...
      <div
        ...
        className={`bg-white cursor-pointer px-2 h-9 border rounded-lg flex justify-between items-center transition-all ${isOpen ? "border-[#0192ED] ring-1 ring-[#0192ED]/20" : "border-gray-300 hover:border-gray-300"}`}
      >
        <span className={`truncate text-[10px] sm:text-[12px] ${selectedLabel ? "text-gray-700 font-[600]" : "text-gray-300"}`}>
          {selectedLabel || placeholder}
        </span>
        <ChevronDown
          className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${isOpen ? "rotate-180 text-[#0192ED]" : ""}`}
        />
      </div>
          ...
          {options.length > 0 ? (
            options.map((option) => (
              <li
                ...
                className={`px-3 py-2 text-[11px] font-[500] transition-colors cursor-pointer ${selected === option.value ? "bg-blue-50 text-[#0192ED]" : "text-gray-600 hover:bg-gray-50"}`}
              >
                {option.label}
              </li>
            ))
