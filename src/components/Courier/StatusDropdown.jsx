  ...
  return (
    <div className="relative w-full flex flex-col gap-1" ref={dropdownRef}>
      ...
      <div
        ...
        className={`bg-white cursor-pointer px-2 h-9 border rounded-lg flex justify-between items-center transition-all ${isOpen ? "border-[#0192ED] ring-1 ring-[#0192ED]/20" : "border-gray-200"}`}
      >
        ...
        <ul className="absolute z-[70] top-full left-0 w-full bg-white border border-gray-100 mt-1 rounded-lg shadow-lg max-h-56 overflow-y-auto animate-popup-in py-1">
          {options.map((option) => (
            <li
              ...
              className={`px-3 py-2 text-[10px] sm:text-[12px] font-[600] transition-colors cursor-pointer ${Status === option ? "bg-blue-50 text-[#0192ED]" : "text-gray-600 hover:bg-gray-50"}`}
            >
              {option}
            </li>
          ))}
