      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        {options.map((opt) => (
          <button
            ...
            className={`flex items-center justify-between w-full p-3 rounded-lg border transition-all duration-300 ${selected === opt.label ? "border-[#0192ED] bg-blue-50" : "border-gray-200 hover:border-gray-300"
              }`}
          >
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-[#0192ED]">{opt.icon}</div>
              <span className="text-sm font-semibold text-gray-700">{opt.label}</span>
            </div>
            {selected === opt.label && (
              <div className="w-5 h-5 rounded-full bg-[#0192ED] flex items-center justify-center text-white text-xs">
                <FaCheck />
              </div>
            )}
          </button>
          ...
          {!isEditing ? (
            <>
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  onClick={handleEdit}
                  className="px-4 py-1.5 text-sm border border-[#0192ED] text-[#0192ED] rounded-lg hover:bg-[#0192ED] hover:text-white transition"
                >
                  Edit
                </button>
              </div>
              <div className="grid max-h-[300px] overflow-y-auto sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {allCouriers.map((c, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* {priorityIndex !== -1 && ( */}
                      <span className="absolute top-0 left-0 bg-[#0192ED] text-white text-[10px] font-semibold px-1.5 py-0.5 rounded-tl-lg rounded-br-md">
                        {i + 1}ᵗʰ
                      </span>
                      {/* )} */}
            ...
            <DragDropContext onDragEnd={handleDragEnd}>
              ...
              <div className="flex justify-end gap-2 mt-2">
                ...
                <button
                  onClick={handleSave}
                  className="bg-[#0192ED] text-[12px] hover:opacity-90 transition-all text-white px-3 py-2 rounded-lg font-[600]"
                >
                  Save
                </button>
              </div>
