    ...
    <section className="w-full mx-auto sm:px-2">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="flex items-center gap-2 px-3 py-1.5 flex-1 sm:w-64 border border-gray-300 rounded-lg focus-within:border-[#0192ED] group shadow-sm bg-white transition-all h-9">
            <Search className="w-3.5 h-3.5 text-gray-400 group-focus-within:text-[#0192ED] transition-colors" />
            <input
              type="text"
          ...
          <button
            ...
            className="px-3 py-2 text-[10px] sm:text-[12px] font-[600] bg-[#0192ED] text-white rounded-lg hover:bg-opacity-90 transition shadow-sm whitespace-nowrap h-9"
          >
            Add Cutoff Time
          </button>
        </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {data.filter(item =>
                ...
                <tr key={item._id} className="text-gray-700 hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 whitespace-nowrap font-medium text-[#0192ED]">{formatTo12H(item?.cutoffTime)}</td>
                  <td className="px-3 py-2 whitespace-nowrap flex space-x-2">
                    <button
                      onClick={() => setEditItem(item)}
                      className="p-1 text-white hover:text-gray-100 hover:bg-blue-500 rounded-full bg-[#0192ED] transition"
                      aria-label="Edit"
                      title="Edit"
                    >
            ...
            <div key={item._id} className="bg-white rounded-lg shadow p-3 border">
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => setEditItem(item)}
                    className="p-1 rounded-full bg-[#0192ED] text-white hover:bg-blue-500 transition"
                    aria-label="Edit"
                    title="Edit"
                  >
                    <AiOutlineEdit size={16} />
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <div className="font-[600] text-[#0192ED] text-[12px]">{formatTo12H(item.cutoffTime)}</div>
              </div>
            </div>
          ))}
          {data.filter(item =>
