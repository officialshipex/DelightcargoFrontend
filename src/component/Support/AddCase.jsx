    ...
    <div className="relative w-full" ref={ref}>
      ...
      <button
        ...
        className={`w-full flex justify-between items-center border rounded px-2 py-2 text-[12px] bg-white transition-all duration-200
          ${open ? "ring-2 ring-[#0192ED] bg-blue-50" : ""}
          ${disabled ? "bg-gray-100 cursor-not-allowed" : "hover:bg-blue-50"}
          focus:outline-none focus:ring-2 focus:ring-[#0192ED]
        `}
      ...
      >
        {options.map((opt) => (
          <div
            key={opt}
            className={`px-3 py-2 text-[12px] cursor-pointer hover:bg-blue-50 transition-colors
              ${
                opt === value ? "bg-blue-100 font-semibold text-[#0192ED]" : ""
              }
            `}
            onClick={() => {
              onChange(opt);
...
export default function AddCase({ isOpen, onClose, refresh }) {
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-2 right-2 text-gray-400 hover:text-[#0192ED] bg-gray-100 hover:bg-blue-50 rounded-full p-1 transition z-10"
          aria-label="Close"
        >
          <svg
            width="20"
  ...
  <label className="flex items-center cursor-pointer text-[12px]">
    <input
      ...
      className="accent-[#0192ED] mr-1 focus:ring-0 focus:outline-none"
    ...
    /> <span className="ml-1 text-gray-700">Multiple</span>
        ...
        {awbType === "single" && (
          <div>
            ...
            <input
              type="text"
              className="w-full border rounded px-2 py-2 text-[12px] focus:ring-2 focus:ring-[#0192ED] transition bg-white focus:outline-none"
              value={awbNumbers[0] || ""}
        ...
        {awbType === "multiple" && (
          <div>
            ...
            {awbInputType === "manual" && (
              <textarea
                className="w-full border rounded px-2 py-1 mt-2 text-[12px] focus:ring-2 focus:ring-[#0192ED] transition bg-white focus:outline-none"
                rows={2}
            ...
            {awbInputType === "upload" && (
              <label className="flex flex-col items-start mt-2 w-full">
                ...
                <div className="relative w-full">
                  ...
                  />
                  <div className="flex items-center gap-2 border border-dashed border-[#0192ED] rounded px-3 py-2 bg-blue-50 text-[#0192ED] text-[12px] cursor-pointer hover:bg-blue-100 transition">
                    <svg
        ...
        <div>
          ...
          <textarea
            className="w-full border rounded px-2 py-1 text-[12px] focus:ring-2 focus:ring-[#0192ED] transition bg-white focus:outline-none"
            rows={3}
            value={message}
            ...
            {attachments.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {attachments.map((file, idx) => (
                  <span
                    key={idx}
                    className="bg-blue-100 text-[#0192ED] px-2 py-0.5 rounded text-[11px]"
                  >
                    {file.name}
                  </span>
                ))}
        ...
        <div className="absolute bottom-0 left-0 w-full bg-white border-t border-gray-200 px-4 py-2 flex justify-between md:justify-end gap-2">
          <button
            type="button"
            onClick={resetForm}
            className="border border-gray-300 text-gray-500 px-3 py-2 rounded-lg bg-gray-200 font-semibold text-[12px] hover:bg-blue-50 transition w-1/2 md:w-auto"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="bg-[#0192ED] text-white px-4 py-2 rounded-lg font-semibold text-[12px] hover:bg-blue-600 transition w-1/2 md:w-auto"
          >
            Submit
          </button>
        </div>
