    ...
    <div className="space-y-2">
      ...
      <div className="bg-white border border-gray-100 rounded-lg p-3 shadow-sm relative overflow-hidden">
        {/* <div className="absolute top-0 right-0 p-3 opacity-10 pointer-events-none">
          <ExternalLink className="w-12 h-12 text-[#0192ED]" />
        </div> */}
        ...
        <p className="text-gray-500 text-[11px] max-w-2xl">
          ...
          {commissionPercentage !== null && (
            <span className="block mt-1 font-bold text-[#0192ED]">You currently earn {commissionPercentage}% commission on your referral revenue.</span>
          )}
        </p>
      </div>

      {/* Stats Cards - Mobile View */}
      <div className="sm:hidden bg-white border border-[#0192ED] rounded-lg p-3 shadow-sm space-y-2">
      ...
      <div className="flex flex-col md:flex-row gap-2 items-stretch">
        <div className="flex-1 bg-white border border-gray-200 rounded-lg p-2 flex items-center gap-3 shadow-sm overflow-hidden">
          ...
          <button
            onClick={handleCopy}
            className={`p-2 rounded-lg transition-all flex-shrink-0 ${copied ? "bg-[#0192ED] text-white" : "bg-gray-100 text-[#0192ED] hover:bg-blue-100"}`}
          >
          ...
          <div className="hidden md:block overflow-hidden bg-white shadow-sm">
            <div className="h-[calc(100vh-340px)] overflow-y-auto">
              <table className="w-full border-collapse">
                <thead className="bg-[#0192ED] text-white font-[600] sticky top-0 z-10 text-[12px]">
                  ...
                  {monthlyData.length > 0 ? (
                    monthlyData.map((row, i) => (
                      <tr key={i} className={`border-b border-gray-300 hover:bg-gray-50 transition-colors ${selectedRows.find(r => r._id === row._id) ? "bg-blue-50/50" : ""}`}>
                        ...
                        <td className="py-3 px-3 font-bold text-[#0192ED]">₹{Number(row.shippingCharges || 0).toFixed(2)}</td>
                        <td className="py-3 px-3 font-bold text-[#0192ED]">₹{Number(row.commission || 0).toFixed(2)}</td>
              ...
              monthlyData.map((row, i) => (
                <div key={i} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm animate-popup-in">
                  <div className="flex justify-between items-center mb-2 border-b border-gray-50 pb-2">
                    ...
                    <span className="text-[9px] bg-blue-50 text-[#0192ED] px-2 py-0.5 rounded font-bold uppercase tracking-tight border border-blue-100/50">Active Period</span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-2">
                    ...
                    <div className="bg-blue-50/30 p-1.5 rounded border border-blue-100/20">
                      ...
                      <p className="text-[11px] font-bold text-[#0192ED]">₹{Math.round(row.commission || 0)}</p>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md overflow-hidden animate-popup-in">
            ...
            <div className="p-6 space-y-4">
              <div className="text-center">
                ...
                <p className="text-[24px] font-bold text-[#0192ED]">₹{Number(stats.remaining || 0).toFixed(2)}</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-[#0192ED]/20 focus:border-[#0192ED]"
                />
                <p className="text-[10px] text-gray-400 italic">This amount will be instantly added to your DelightCargo wallet balance.</p>
              </div>
            </div>
            ...
            <div className="px-4 py-3 bg-gray-50 border-t flex gap-2">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-[#0192ED] text-white rounded-lg text-[12px] font-bold hover:bg-blue-600 disabled:opacity-50 transition-all shadow-sm"
              >
