...
const AdminReferral = () => {
  ...
  return (
    <div className="space-y-2 sm:px-2">
      ...
      <div className="sm:hidden bg-white border border-[#0192ED] rounded-lg p-3 shadow-sm space-y-1">
      ...
      <div className="hidden sm:grid grid-cols-2 md:grid-cols-4 gap-2">
        {summaryCards.map((card, i) => (
          <div key={i} className="bg-white border border-[#0192ED] rounded-lg p-3 flex items-center gap-3 shadow-sm hover:shadow-md transition-all">
            <div className="p-2 rounded-full flex justify-center items-center bg-[#0192ED] text-white">
              {card.icon}
            </div>
          ...
          <div className="relative h-full" ref={actionDropdownRef}>
            <button
              ...
              className={`h-9 px-4 rounded-lg bg-white text-[12px] font-bold flex items-center gap-1 border transition-all ${selectedRows.length > 0 || isAnyFilterApplied
                  ? "border-[#0192ED] text-[#0192ED] hover:bg-blue-50 shadow-sm"
            ...
            {isActionDropdownOpen && (
              <div className="absolute left-0 mt-2 w-56 bg-white border border-gray-100 rounded-lg shadow-xl z-50 py-1 overflow-hidden animate-popup-in">
                ...
                >
                  <Wallet className="w-4 h-4 text-[#0192ED]" />
              ...
              <table className="w-full border-collapse">
                  ...
                  <tr className="text-left text-[12px]">
                    <th className="py-2 px-3 text-center">
                      <input
                        ...
                        className="rounded w-3 h-3 border-gray-300 accent-[#0192ED]"
                      />
                    </th>
                ...
                <tbody className="text-[12px] text-gray-700">
                  {data.length === 0 ? (
                    ...
                    data.map((row, i) => (
                      <tr key={i} className={`border-b border-gray-300 hover:bg-gray-50 transition-colors ${selectedRows.find(r => r._id === row._id) ? "bg-blue-50/50" : ""}`}>
                        ...
                        <td className="py-2 px-3">
                          <p className="font-bold text-[#0192ED]">{row.userId || "-"}</p>
                        ...
                        <td className="py-2 px-3 text-right font-bold text-[#0192ED]">
                        ...
                        <td className="py-2 px-3">
                          <button
                            ...
                            className="px-3 py-1 bg-blue-50 hover:bg-blue-100 text-[#0192ED] rounded-lg text-[11px] font-bold transition-all"
                            title="View Details"
                          >
                            Details
                          </button>
                ...
                <div key={i} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm relative animate-popup-in">
                  <div className="flex justify-between items-center mb-2 border-b border-gray-50 pb-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-[#0192ED] font-bold text-[11px] leading-tight">{row.userId || "-"}</p>
                      <h4 className="text-gray-800 font-bold text-[13px] truncate leading-tight">{row.userName}</h4>
                    </div>
                    <button
                      onClick={() => setSelectedReferral(row)}
                      className="px-3 py-1 bg-blue-50 text-[#0192ED] rounded-lg text-[11px] font-bold border border-blue-100 hover:bg-blue-100 transition-all ml-2 h-7 flex items-center shrink-0"
                    >
                      Details
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-2">
                    ...
                    <div className="bg-blue-50/30 p-1.5 rounded border border-blue-100/20">
                      <p className="text-gray-400 text-[9px] font-bold uppercase tracking-tight">Reward</p>
                      <p className="font-bold text-[#0192ED] text-[11px]">₹{Math.round(row.totalCommission || 0)}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-popup-in">
            ...
            <div className="p-6 space-y-6">
                ...
                <UserFilter
                  onUserSelect={(id) => {
                    if (id) {
                      ...
                      // In DelightCargo, UserFilter only gives ID. Let's find the user from suggestions or fetch
              ...
              {selectedUser && (
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 animate-fade-in relative z-0">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="px-2 py-0.5 bg-[#0192ED] text-white text-[10px] font-bold rounded">Target User</span>
                  </div>

                  {isStatsLoading ? (
                    <div className="flex justify-center py-4">
                      <div className="w-6 h-6 border-2 border-[#0192ED] border-t-transparent rounded-full animate-spin"></div>
                    </div>
                  ) : userStats ? (
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div className="bg-white p-2 rounded-lg border border-blue-100">
                        ...
                        <p className="text-[12px] font-bold text-[#0192ED]">₹{userStats.remaining?.toFixed(2)}</p>
                      </div>
                    </div>
                  ) : (
                  ...
                  {userStats && (
                    <div className="mt-6 space-y-3">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2 border border-blue-200 rounded-lg text-[14px] font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0192ED]/20 transition-all"
                      />
                      <button
                        ...
                        className="w-full py-2.5 bg-[#0192ED] text-white rounded-lg text-[12px] font-bold hover:bg-blue-600 transition-all shadow-md disabled:opacity-50"
                      >
