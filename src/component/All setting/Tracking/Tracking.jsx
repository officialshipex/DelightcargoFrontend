...
function Tracing2() {
  ...
  const CopyableText = ({ label, text, copyKey }) => {
    ...
    return (
      ...
      >
        {label} :{" "}
        <span className="font-[600] text-[#0192ED] truncate max-w-[150px] sm:max-w-full">
          {text || "-"}
        </span>
        {copiedField === copyKey ? (
          <Check size={14} className="text-blue-500 flex-shrink-0" />
        ) : (
          <Copy size={14} className="text-[#0192ED] hover:text-blue-500 flex-shrink-0" />
        )}
      </span>
    );
  };
    ...
    <div className="relative bg-[#f7fafc] min-h-screen py-4 px-1 sm:px-4">
      <div className="max-w-4xl mx-auto flex flex-col items-center">
        <div className="my-4 flex flex-col gap-2 w-full items-center">
          <img className="w-40 h-14 -mt-[15px]" src={DelightCargo} alt="DelightCargo" />
          {admin && adminTab && (
            <div className="w-full">
              <div className="flex w-full flex-col">
                <div className="flex w-full gap-2">
                  ...
                  />
                  <button
                    className="px-3 py-2 sm:w-[14%] w-[28%] border border-gray-100 bg-[#0192ED] sm:h-12 h-10 text-white font-[600] text-[10px] sm:text-[12px] rounded-lg hover:bg-blue-500 transition"
                    onClick={handleTrack}
                    disabled={loading}
                  >
    ...
    <div className="bg-white rounded-lg shadow-sm border-2 mx-auto w-full mb-6">
      ...
      <div className="flex flex-row justify-between items-center px-3 pt-4 pb-2 gap-2">
        <div className="flex flex-col font-[600] text-[10px] gap-1 items-start sm:text-[12px] text-gray-700">
          ...
          <span>
            Status :{" "}
            <span
              className={`text-[10px] px-2 py-0.5 rounded ${isCancelled ? "bg-red-100 text-red-600" : "text-[#0192ED] bg-blue-100"
                }`}
            >
              {trackingAwb.status}
            </span>
      ...
      <div className="px-3 pb-4">
        <div className="font-[600] text-gray-700 text-[12px] sm:text-[14px] my-2 flex items-center gap-2">
          <FontAwesomeIcon icon={faMapMarkerAlt} className="text-[#0192ed]" />
          Tracking Details
        </div>
              ...
              return (
                <div key={idx} className="flex gap-3 py-2 relative">
                  ...
                  ${showCancelledRed
                          ...
                          : idx === 0
                            ? "bg-[#0192ED] border-[#0192ED]"
                            : "bg-blue-200 border-[#0192ED]"
                        }
                  hover:scale-110 hover:ring-2 hover:ring-[#0192ED] cursor-pointer`}
                    ></button>
                    ...
                    ${showCancelledRed ? "bg-red-600" : "bg-[#0192ED]"}`}
                      ></div>
                    )}
                  </div>
