          ...
          <div className="bg-white p-4 rounded-lg shadow-md w-full md:w-3/5 text-gray-700 space-y-4">
            ...
            <div className="space-y-2 text-[10px] font-[600] text-gray-500 sm:text-[12px]">
              <label className="flex font-[600] text-gray-500 items-center gap-2">
                <input type="checkbox" checked={settings.showLogoOnLabel} onChange={() => handleChange(null, "showLogoOnLabel")} className="accent-[#0192ED] w-4" />
                Show Logo on Label
              </label>
              ...
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={settings.hideCustomerMobile} onChange={() => handleChange(null, "hideCustomerMobile")} className="accent-[#0192ED] w-4" />
                Hide Customer Mobile Number
              </label>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={settings.hideOrderBarcode} onChange={() => handleChange(null, "hideOrderBarcode")} className="accent-[#0192ED] w-4" />
            ...
            <div className="grid grid-cols-2 gap-2 sm:text-[12px] font-[600] text-gray-500 text-[10px]">
              {Object.entries(settings.warehouseSettings).map(([key, val]) => (
                <label key={key} className="flex items-center gap-2 capitalize">
                  <input type="checkbox" checked={val} onChange={() => handleChange("warehouseSettings", key)} className="accent-[#0192ED] w-4" />
                  {key.replace(/([A-Z])/g, " $1")}
                </label>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:text-[12px] font-[600] text-gray-500 text-[10px]">
              {Object.entries(settings.productDetails).map(([key, val]) => (
                <label key={key} className="flex items-center gap-2 capitalize">
                  <input type="checkbox" checked={val} onChange={() => handleChange("productDetails", key)} className="accent-[#0192ED] w-4" />
                  {key.replace(/([A-Z])/g, " $1")}
                </label>
              ))}
            </div>

            <button onClick={handleSave} className="bg-[#0192ED] sm:text-[12px] font-[600] text-[10px] text-white px-3 py-2 rounded-lg hover:bg-blue-500 mt-4">
              Save
            </button>
          </div>
            ...
            <div className="border border-black p-4 space-y-2">
              ...
              <div className="border-t border-black pt-2 flex justify-between items-center">
                ...
                <div className="text-center" style={{ lineHeight: "1.1" }}>
                  <p className="font-[600]">DELIGHT CARGO</p>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            >
              <div className="mt-1">
                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                  settings.labelSize === "A4" ? "border-[#0192ED]" : "border-gray-400"
                }`}>
                  {settings.labelSize === "A4" && (
                    <div className="w-2 h-2 rounded-full bg-[#0192ED]" />
                  )}
                </div>
              </div>
              <div className="flex-1">
                <p className={`text-[12px] font-[700] ${settings.labelSize === "A4" ? "text-[#0192ED]" : "text-gray-700"}`}>
            ...
            >
              <div className="mt-1">
                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                  settings.labelSize === "thermal" ? "border-[#0192ED]" : "border-gray-400"
                }`}>
                  {settings.labelSize === "thermal" && (
                    <div className="w-2 h-2 rounded-full bg-[#0192ED]" />
                  )}
              ...
              <div className="flex-1">
                <p className={`text-[12px] font-[700] ${settings.labelSize === "thermal" ? "text-[#0192ED]" : "text-gray-700"}`}>
          ...
          <div className="flex justify-start mt-4">
            <button
              onClick={handleSave}
              className="bg-[#0192ED] text-white text-[12px] font-[600] py-2 px-3 rounded-lg hover:bg-opacity-90 transition-colors shadow-sm"
            >
              Submit
            </button>
          </div>
