            ...
            {loading ? (
              ...
              logs.map((log) => (
                <tr key={log._id} className="border-b border-gray-300 hover:bg-gray-50 text-[12px] text-gray-600">
                  ...
                  <td className="py-2 px-3 max-w-xs truncate text-[#0192ED]" title={log.url}>{log.url}</td>
                  ...
                  <td className="py-2 px-3 text-center">
                    <span className={`font-semibold ${log.httpStatus < 300 ? "text-blue-600" : "text-red-600"}`}>
                      {log.httpStatus || "-"}
                    </span>
                  </td>
                  <td className="py-2 px-3 text-center">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                      log.status === "Success" ? "bg-blue-100 text-[#0192ED] border-blue-200" : "bg-red-100 text-red-600 border-red-200"
                    }`}>
                      {log.status}
                    </span>
                  </td>
                  <td className="py-2 px-3 text-center">
                    <button 
                      ...
                      className="text-[#0192ED] hover:underline font-[600]"
                    >
                      Details
                    </button>
                  </td>
              ...
              {logs.map((log) => (
                <div key={log._id} className="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm hover:shadow transition-all duration-200">
                  ...
                  <div className="flex justify-between items-center mb-1 border-b border-gray-100 pb-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[10px] font-bold ${log.httpStatus < 300 ? "text-blue-600" : "text-red-600"}`}>
                        HTTP {log.httpStatus || "N/A"}
                      </span>
                      <span className={`px-1.5 py-0.2 rounded text-[9px] font-bold border ${
                        log.status === "Success" ? "bg-blue-50 text-[#0192ED] border-blue-100" : "bg-red-50 text-red-600 border-red-100"
                      }`}>
                        {log.status}
                      </span>
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="text-[#0192ED] hover:underline text-[10px] font-semibold"
                    >
                      View Details →
                    </button>
                  </div>
            ...
            <div className="p-5 overflow-auto max-h-[80vh] space-y-4">
              <div className="grid grid-cols-2 gap-4 text-[12px]">
                ...
                <div className="bg-gray-50 p-2 rounded border col-span-2">
                  ...
                  <p className="text-[#0192ED] font-mono break-all">{selectedLog.url}</p>
                </div>
                <div className="bg-gray-50 p-2 rounded border">
                  <p className="text-gray-400 text-[10px] uppercase font-bold">HTTP Status</p>
                  <p className={`font-bold ${selectedLog.httpStatus < 300 ? "text-blue-600" : "text-red-600"}`}>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => copyToClipboard(JSON.stringify(selectedLog.payload, null, 2), "payload")} className="text-gray-400 hover:text-[#0192ED]">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => copyToClipboard(JSON.stringify(selectedLog.response, null, 2), "response")} className="text-gray-400 hover:text-[#0192ED]">
