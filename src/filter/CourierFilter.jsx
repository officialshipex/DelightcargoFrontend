    ...
    <div className={`relative w-full ${width}`} ref={dropdownRef}>
      <button
        ...
        className={`w-full bg-white py-2 px-3 text-[12px] font-[600]
          border rounded-lg flex items-center justify-between text-gray-400 ${showDropdown || selectedCourier.length > 0 ? "border-[#0192ED] text-[#0192ED]" : "border-gray-300"}`}
      >
              ...
              return (
                <div
                  ...
                  className={`px-3 py-2.5 text-[12px] font-[600] flex items-center gap-2
                             cursor-pointer transition-colors ${isSelected ? "text-[#0192ED] bg-blue-50" : "text-gray-500 hover:bg-gray-50"}`}
                  onClick={() => handleToggle(courier)}
                >
                  <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-[#0192ED] border-[#0192ED]" : "border-gray-300 bg-white"}`}>
